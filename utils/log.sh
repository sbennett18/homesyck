...
class bcolors:
    ...
    RED     = "\e[1;31m" # Red - error
    GREEN   = "\e[1;32m" # Green - success
    BLUE    = "\e[1;34m" # Blue - no action/ignored
    CYAN    = "\e[1;36m" # Cyan - pending action
    WHITE   = "\e[1;37m" # White - info

def err(exit_status, reason):
  if pending_status:
      fail()
  status(bcolors.RED, "error", reason, sys.stderr)
